...
Now, we have a :class:`TableList <camelot_hybrid.core.TableList>` object called ``tables``, which is a list of :class:`Table <camelot_hybrid.core.Table>` objects. We can get everything we need from this object.
...
Woah! The accuracy is top-notch and there is less whitespace, which means the table was most likely extracted correctly. You can access the table as a pandas DataFrame by using the :class:`table <camelot_hybrid.core.Table>` object's ``df`` property.
...
Looks good! You can now export the table as a CSV file using its :meth:`to_csv() <camelot_hybrid.core.Table.to_csv>` method. Alternatively you can use :meth:`to_json() <camelot_hybrid.core.Table.to_json>`, :meth:`to_excel() <camelot_hybrid.core.Table.to_excel>` :meth:`to_html() <camelot_hybrid.core.Table.to_html>` or :meth:`to_sqlite() <camelot_hybrid.core.Table.to_sqlite>` methods to export the table as JSON, Excel, HTML files or a sqlite database respectively.
...
You can also export all tables at once, using the :class:`tables <camelot_hybrid.core.TableList>` object's :meth:`export() <camelot_hybrid.core.TableList.export>` method.
...
.. tip::
    ...
    ::
        ...
        $ camelot_hybrid --format csv --output foo.csv lattice foo.pdf
...
By default, Camelot only uses the first page of the PDF to extract tables. To specify multiple pages, you can use the ``pages`` keyword argument::
    ...
    >>> camelot_hybrid.read_pdf('your.pdf', pages='1,2,3')

.. tip::
    Here's how you can do the same with the :ref:`command-line interface <cli>`.
    ::

        $ camelot_hybrid --pages 1,2,3 lattice your.pdf
...
To extract tables from encrypted PDF files you must provide a password when calling :meth:`read_pdf() <camelot_hybrid.read_pdf>`.

::

    >>> tables = camelot_hybrid.read_pdf('foo.pdf', password='userpass')
    >>> tables
    <TableList n=1>
...
.. tip::
    ...
    ::
        ...
        $ camelot_hybrid --password userpass lattice foo.pdf
...
Further encryption support may be added in future, however in the meantime if your PDF files are using unsupported encryption algorithms you are advised to remove encryption before calling :meth:`read_pdf() <camelot_hybrid.read_pdf>`. This can been successfully achieved with third-party tools such as `QPDF`_.
