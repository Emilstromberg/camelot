...
def test_unknown_flavor():
    ...
    with pytest.raises(NotImplementedError, match=message):
        camelot_hybrid.read_pdf(filename, flavor='chocolate')


def test_input_kwargs():
    message = "columns cannot be used with flavor='lattice'"
    with pytest.raises(ValueError, match=message):
        camelot_hybrid.read_pdf(filename, columns=['10,20,30,40'])
...
def test_lattice_no_tables_on_page():
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        with pytest.raises(UserWarning) as e:
            tables = camelot_hybrid.read_pdf(filename, flavor="lattice")
...
def test_no_tables_found_logs_suppressed():
    ...
    with warnings.catch_warnings():
        ...
        warnings.simplefilter("error")
        try:
            camelot_hybrid.read_pdf(filename, suppress_stdout=True)
        except Warning as e:
            warning_text = str(e)
...
def test_bad_password():
    ...
    message = "file has not been decrypted"
    with pytest.raises(Exception, match=message):
        camelot_hybrid.read_pdf(filename, password='wrongpass')
